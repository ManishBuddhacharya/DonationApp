...
<div class="panel panel-default panel-border-color panel-border-color-primary">
  <div class="panel-heading panel-heading-divider">Edit Blog</div>
  <div class="panel-body">
    <form id="form_blog">
      <div class="col-md-12">
        <div class="form-group xs-pt-10">
          <label>Title</label>
          <input type="text" name="title" placeholder="Title" value="{{$blog->title}}" class="input-xs form-control validate">
        </div>
      </div>
      <div class="col-md-4">
      ...
      <div class="col-md-12">
        <div class="form-group xs-pt-10">
          <label>Content</label>
          <textarea name="content" id="summernote" class="form-control validate">{{$blog->content}}</textarea>
        </div>
      </div>
      ...
      <div class="row xs-pt-15">
        <div class="col-xs-6 pull-right mr-10">
          <p class="text-right">
            <button type="submit" class="btn btn-space btn-primary" id="update_blog">Update</button>
  ...
  function fileValidation(type)
{
  ...
  $(document).off('click', '#update_blog').on('click','#update_blog', function(e){
    e.preventDefault();
    if (validate() === 0){
      var formData = new FormData($('#form_blog')[0]);        
      saveUpdateAction({
          url : '/backend/blog/update/{{$blog->id}}',
          data: formData,
          contentType : false,
          processData: false,
          hasCb: true,            
